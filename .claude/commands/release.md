...
`$ARGUMENTS` says which part of the version moves. It is one of `patch`,
`minor` or `major`. **If it says none of those, ask which — do not guess.** The
keyword decides the number; it does not decide whether the release is worth
cutting.
